        ...
        {signals.map((s) => (
          <div key={s.id} className="card p-4">
            <div className="flex justify-between items-start gap-4">
              ...
              <span className={`badge badge-${s.decision === "paper_copy" ? "copy" : s.decision}`}>{s.decision}</span>
            </div>
            <div className="grid grid-cols-3 gap-4 mt-3 text-xs">
              <div>
                <div className="text-muted">Entry price</div>
                <div>{s.walletEntryPrice}</div>
              </div>
              <div>
                <div className="text-muted">Detected price</div>
                <div>{s.detectedPrice}</div>
              </div>
              <div>
                <div className="text-muted">Copy score</div>
                <div>{s.copyScore !== null ? (s.copyScore * 100).toFixed(0) : "—"}</div>
              </div>
            </div>
